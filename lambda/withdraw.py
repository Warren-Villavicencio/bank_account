...
def handler(event, context):
    ...
    try:
        ...
        current_balance = account.get('saldo', Decimal('0'))
        ...
        if 'correoelectronico' in account:
            ...
            return {
                'statusCode': 200,
                'body': json.dumps('Retiro realizado y correo enviado')
            }
        else:
            print(f"Advertencia: No se encontró correo electrónico para la cuenta {account_id}")
            return {
                'statusCode': 200,
                'body': json.dumps('Retiro realizado, pero no se pudo enviar correo (correo no encontrado)')
            }
